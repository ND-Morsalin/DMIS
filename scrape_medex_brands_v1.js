...
/**
 * Extract entries using the HTML structure you provided:
 * <a class="hoverable-block"> 
 *   <div class="row data-row">
 *     <div class="col-xs-12 data-row-top"> <span class="md-icon-container"><img ...></span> A-Cal D </div>
 *     <div class="col-xs-12 data-row-strength"><span class="grey-ligten">500 mg+200 IU</span></div>
 *     <div class="col-xs-12"> Elemental Calcium + Vitamin D3 </div>
 *     <div class="col-xs-12"><span class="data-row-company">ACME Laboratories Ltd.</span></div>
 *   </div>
 * </a>
 */
...
(async function main() {
  ...
  for (let page = 1; page <= MAX_PAGES; page += CONCURRENCY) {
    ...
    const batchPromises = pages.map((p, idx) => (async () => {
    ...
    console.log(`Fetched pages ${page}..${Math.min(page + CONCURRENCY - 1, MAX_PAGES)} — collected items so far: ${all.length}`);
  ...
  const map = new Map();
  for (const e of all) {
    const key = ((e.name || "") + "||" + (e.company || "")).toLowerCase().trim();
