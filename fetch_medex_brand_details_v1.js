...
function getMedicineId(item, source_url, fallbackIndex) {
  ...
  if (item._id && typeof item._id === "object") {
    ...
    try { return String(item._id); } catch (e) {}
  }
  if (item._id) return String(item._id);
  if (item.id) return String(item.id);
  // last fallback: deterministic hash
  const seed = source_url || JSON.stringify(item) || String(fallbackIndex || "");
  return crypto.createHash("sha1").update(seed).digest("hex").slice(0, 24);
}
...
function parseBrandDetail(html, pageUrl) {
  ...
  for (const p of packageEntries) {
    ...
    if (!seenPK.has(key)) { seenPK.add(key); packages.push(p); }
  }
  ...
  const pricing = { unit_price: unit_price || null, strip_price: strip_price || null, pack_size_info: pack_size_info || null, packages };
  ...
  function parseSectionToStructuredArray(sectionId) {
    ...
    function flushCurrent() {
      ...
      if (currTitle !== null || infosText || itemsArr.length) {
        out.push({ title: currTitle || null, information: infosText || null, items: itemsArr });
      }
  ...
  const promoteSections = [
    "indications","mode_of_action","interaction","contraindications","side_effects",
    "pregnancy_cat","precautions","pediatric_uses","overdose_effects","storage_conditions",
